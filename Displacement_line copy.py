...
def process_frame(frame, reference_line_x):
    ...
    if results.pose_landmarks:
        # Drawing the pose landmarks and connections
...
def process_frame(frame, reference_line_x):
    ...
    if results.pose_landmarks:
        ...
        if left_hip is not None and right_hip is not None and left_shoulder is not None and right_shoulder is not None:
            ...
            cv.circle(frame, tuple(adjusted_midpoint), 5, (255, 255, 0), -1)
        ...
        for connection in limb_connections:
            ...
            if results.pose_landmarks.landmark[start_idx].visibility > 0.5 and results.pose_landmarks.landmark[end_idx].visibility > 0.5:
                ...
                cv.line(frame, start, end, (250, 200, 10), 3)
...
video_path1 = '/Users/jolie/CODING/MOVEMENT TRACING/video/IMG_0803 2.MOV'
video_path2 = '/Users/jolie/CODING/MOVEMENT TRACING/video/IMG_0803.mov'
...
while True:
    ...
    if key == ord(' '):  # Space bar pressed
        ...
        while paused:
            key = cv.waitKey(30) & 0xFF  # Wait for a key event
            if key == ord(' '):  # If 'p' is pressed again, unpause the video
                paused = not paused
